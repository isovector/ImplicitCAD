...
import qualified Graphics.Implicit.Primitives as Prim (withRounding, sphere, rect3, rect, translate, circle, polygon, extrude, cylinder2, union, intersect, difference, rotate, rotate3V, rotate3, scale, extrudeM, rotateExtrude, shell, pack3, pack2)
...
union = moduleWithSuite "union" $ \_ children -> do
    ...
    pure $ pure $
        objReduce (Prim.withRounding r . Prim.union) (Prim.withRounding r . Prim.union) children
...
intersect = moduleWithSuite "intersection" $ \_ children -> do
    ...
    pure $ pure $
        objReduce (Prim.withRounding r . Prim.intersect) (Prim.withRounding r . Prim.intersect) children
...
difference = moduleWithSuite "difference" $ \_ children -> do
    ...
    pure $ pure $
        objReduce (Prim.withRounding r . unsafeUncurry Prim.difference) (Prim.withRounding r . unsafeUncurry Prim.difference) children
  where
